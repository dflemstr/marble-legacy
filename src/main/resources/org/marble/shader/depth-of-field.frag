...
float bias = 0.2; //bokeh edge bias
...
float bdepth(vec2 coords) {
    ...
    int i;
    for (i = 0; i < 9; i++) {
        float tmp = texture2D(normalDepths, coords + offset[i]).a;
        d += tmp * kernel[i];
    }
...
void main() 
{
    ...
    float depth = texture2D(normalDepths, coord).a;
    ...
    if (autofocus) {
        fDepth = texture2D(normalDepths, focus).a;
    }
