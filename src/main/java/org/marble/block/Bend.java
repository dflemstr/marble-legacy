...
public class Bend extends AbstractEntity implements Connected, Graphical,
        Physical {
    ...
    public Bend(final double length, final double angle) {
        this(length, 0.7, 0.3, angle);
    }

    public Bend(final double width, final double height, final double depth,
            final double angle) {
        this.width = width;
        this.height = height;
        this.depth = depth;
        ...
        left =
                new org.marble.shape.Bend("left rail", 100, 100, width / 2
                        + height / 2, depth / 2, angle, false);
        right =
                new org.marble.shape.Bend("right rail", 100, 100, width / 2
                        - height / 2, depth / 2, angle, false);
    ...
    public Map<String, Connector> getConnectors() {
        return Connectors.fromBend(width, height, depth, angle);
    }
